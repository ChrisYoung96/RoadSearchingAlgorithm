    ...
    def __extend_around(self, cur_node):
        ...
        for x, y in zip(xs, ys):
            ...
            if not self.__is_valid_coordinate(new_x, new_y):
                continue
            new_cost = cur_node.cost + self.__cal_cost(new_x, new_y)
            new_node = Node(new_x, new_y, cur_node, new_cost)
            if self.__is_visited(new_node):
                continue
            idx = self.__is_in_open(new_node)
            if idx != -1:
                if new_node.cost < self.__open[idx].cost:
                    self.__open[idx].parent = cur_node
                    self.__open[idx].cost = new_node.cost
                continue
